class PieChart {
  ...
  <tr><th>Name</th><th>Value</th></tr>
    ...
    setLayouts() {
      ...
      const arc = d3
        .arc()
        .innerRadius(calc.innerRadius *1.5)
        .outerRadius(calc.radius *1.5)
        .padAngle(0.02)
        .cornerRadius(1);
    ...
    drawSvgAndWrappers() {
      ...
      var chart = svg
        ...
        .attr(
          'transform',
          'translate(' + calc.chartLeftMargin/3 + ',' + calc.chartTopMargin + ')'
        );
    ...
    drawSlices() {
      ...
      ${arcLabel.centroid(this.correct(d))[1] * 1.25 + d.yOffset}
          `} else {
            ...
            ${arc.centroid(d)[0] * 0.6},
            ${arc.centroid(d)[1] * 1.25}
            ${arcLabel.centroid(this.correct(d))[0] * 0.65 + d.xOffset},
